...
- **New:** Android environment pre-flight check (`checkAndroidEnvironment()`). Fires for the `android` target and the `reactnative` target when chat-component-rn is detected, catching three issues that commonly cause first-time Android builds to fail with opaque errors: (a) `JAVA_HOME` unset → gradle errors with "Unable to locate a Java Runtime"; (b) `ANDROID_HOME` unset → `adb` / `emulator` don't resolve; (c) no emulator running → `expo run:android` errors with "No connected device". Each warning includes the exact fix. When `JAVA_HOME` is missing but Android Studio's bundled JBR exists at the standard macOS path, the suggested `export JAVA_HOME=...` points at the JBR (matches the chat-component-rn README recommendation). When env-var exports are needed, the tool offers to append them to `~/.zshrc` idempotently (uses a marker comment so repeat runs are no-ops). When no emulator is running but AVDs exist, the launch command is printed.
